...
/* ═══════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════ */
const STRIKES = {
  jab: { label: "Jab", short: "1", type: "punch", target: "head", power: 2, speed: 9, color: "#FF4136", side: "lead" },
  cross: { label: "Cross", short: "2", type: "punch", target: "head", power: 7, speed: 7, color: "#FF6B35", side: "rear" },
  leadHook: { label: "Lead Hook", short: "3", type: "punch", target: "head", power: 6, speed: 6, color: "#FF4136", side: "lead" },
  rearHook: { label: "Rear Hook", short: "4", type: "punch", target: "head", power: 8, speed: 5, color: "#FF6B35", side: "rear" },
  leadUppercut: { label: "Lead Uppercut", short: "5", type: "punch", target: "body", power: 6, speed: 5, color: "#FF4136", side: "lead" },
  rearUppercut: { label: "Rear Uppercut", short: "6", type: "punch", target: "body", power: 8, speed: 4, color: "#FF6B35", side: "rear" },
  leadElbow: { label: "Lead Elbow", short: "E1", type: "elbow", target: "head", power: 9, speed: 6, color: "#E8175D", side: "lead" },
  rearElbow: { label: "Rear Elbow", short: "E2", type: "elbow", target: "head", power: 9, speed: 5, color: "#E8175D", side: "rear" },
  teep: { label: "Teep", short: "T", type: "kick", target: "body", power: 3, speed: 7, color: "#00D4FF", side: "lead" },
  leadKick: { label: "Lead Kick", short: "LK", type: "kick", target: "legs", power: 5, speed: 6, color: "#00D4FF", side: "lead" },
  rearKick: { label: "Rear Low Kick", short: "RLK", type: "kick", target: "legs", power: 7, speed: 5, color: "#0074D9", side: "rear" },
  rearBodyKick: { label: "Rear Body Kick", short: "RBK", type: "kick", target: "body", power: 8, speed: 4, color: "#0074D9", side: "rear" },
  headKick: { label: "Head Kick", short: "HK", type: "kick", target: "head", power: 10, speed: 3, color: "#0074D9", side: "rear" },
  leadKnee: { label: "Lead Knee", short: "K1", type: "knee", target: "body", power: 7, speed: 5, color: "#B10DC9", side: "lead" },
  rearKnee: { label: "Rear Knee", short: "K2", type: "knee", target: "body", power: 9, speed: 4, color: "#B10DC9", side: "rear" },
};

const CLASSIC_COMBOS = [
  { name: "Bread & Butter", keys: ["jab", "cross"], desc: "Fundamental 1-2" },
  { name: "Dutch Destroyer", keys: ["jab", "cross", "leadHook", "rearKick"], desc: "Classic Dutch combo" },
  { name: "Question Mark", keys: ["rearBodyKick", "headKick"], desc: "Feint low, go high" },
  { name: "Clinch Entry", keys: ["jab", "cross", "leadHook", "rearKnee"], desc: "Box into clinch" },
  { name: "Elbow Blitz", keys: ["jab", "cross", "leadElbow", "rearElbow"], desc: "Close range carnage" },
  { name: "Body Snatcher", keys: ["jab", "rearUppercut", "leadHook", "rearBodyKick"], desc: "Attack the body" },
  { name: "Leg Eater", keys: ["jab", "rearKick", "cross", "leadKick"], desc: "Chop the legs" },
];
...
function createScene(canvas, width, height) {
  ...
  scene.fog = new THREE.FogExp2(0x080808, 0.25);

  const camera = new THREE.PerspectiveCamera(38, width / height, 0.1, 50);
  camera.position.set(0.6, 1.35, 2.8);
  camera.lookAt(0, 1.1, 0);

  // Lights
  const keyLight = new THREE.SpotLight(0xfff0e0, 1.8, 12, Math.PI / 5, 0.4);
  ...
  const rimLight = new THREE.PointLight(0xff3322, 0.8, 6);
  rimLight.position.set(-1.5, 1.5, -2);
  scene.add(rimLight);

  const rimLight2 = new THREE.PointLight(0x0066ff, 0.5, 6);
  rimLight2.position.set(2, 0.5, -1.5);
  scene.add(rimLight2);

  const ambLight = new THREE.AmbientLight(0x222233, 0.3);
  scene.add(ambLight);

  // Floor — octagon
  const floorGeo = new THREE.CircleGeometry(2.2, 8);
  const floorMat = new THREE.MeshStandardMaterial({
    color: 0x111111, roughness: 0.95, metalness: 0.05
  });
  ...
  const ringMat = new THREE.MeshBasicMaterial({ color: 0xcc1a00, side: THREE.DoubleSide, transparent: true, opacity: 0.3 });
...
export default function StrikingLab3D() {
  ...
  const animRef = useRef({ playing: false, queue: [], currentAnim: null, time: 0, idleSway: 0 });
  ...
  useEffect(() => {
    ...
    try {
      ...
      const tick = (now) => {
        ...
        const orbitAngle = a.idleSway * 0.08;
        camera.position.x = 0.6 + Math.sin(orbitAngle) * 0.3;
        camera.position.z = 2.8 + Math.cos(orbitAngle) * 0.15 - 0.15;
        camera.lookAt(0, 1.1, 0);
  ...
  useEffect(() => {
    if (threeRef.current) {
      threeRef.current.root.scale.x = stance === "southpaw" ? -1 : 1;
    }
  }, [stance]);
  ...
  const playCombo = useCallback(() => {
    ...
    const a = animRef.current;
    a.queue = combo.slice(1).map((k, i) => ({ key: k, index: i + 1 }));
    a.currentAnim = combo[0];
    a.time = 0;
    a.playing = true;
    a.onStrike = (idx, key) => {
      setActiveIdx(idx);
        ...
        <div style={{ flex: "1 1 320px", minWidth: 280, maxWidth: 420 }}>
          ...
          <div style={{ display: "flex", gap: 3, marginBottom: 8, flexWrap: "wrap" }}>
            {[
              ...
              { id: "punch", label: "PUNCHES", c: "#FF4136" },
              { id: "kick", label: "KICKS", c: "#00D4FF" },
              { id: "elbow", label: "ELBOWS", c: "#E8175D" },
              { id: "knee", label: "KNEES", c: "#B10DC9" },
            ].map(cat => (
